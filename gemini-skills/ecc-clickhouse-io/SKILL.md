...
```sql
-- âœ… GOOD: Use indexed columns first
SELECT *
...
-- âŒ BAD: Filter on non-indexed columns first
SELECT *
FROM markets_analytics
WHERE volume > 1000
...
```sql
-- âœ… GOOD: Use ClickHouse-specific aggregation functions
SELECT
...
-- âœ… Use quantile for percentiles (more efficient than percentile)
SELECT
