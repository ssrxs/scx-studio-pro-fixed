...
export function comparePoseProfiles(pose1: PoseProfile, pose2: PoseProfile): { similarity: number; differences: string[] } {
  ...
  else differences.push(`Yaw: ${pose1.headPosition.yaw}Â° vs ${pose2.headPosition.yaw}Â°`);

  if (pose1.headPosition.pitch === pose2.headPosition.pitch) matchingFields++;
  else differences.push(`Pitch: ${pose1.headPosition.pitch}Â° vs ${pose2.headPosition.pitch}Â°`);

  if (pose1.headPosition.roll === pose2.headPosition.roll) matchingFields++;
  else differences.push(`Roll: ${pose1.headPosition.roll}Â° vs ${pose2.headPosition.roll}Â°`);
